...
## 14. Notion ダッシュボード連携（notion-sync）

Vercel Cron の日次レポート自動保存（§12）の後に、Notion のダッシュボード DB へも同じデータを書き込む。

### 14.1 概要

| 項目 | 内容 |
|---|---|
| トリガー | `/api/cron/daily-report` の DB 保存成功後 |
| 対象 DB | ユーザーが Notion で作成したダッシュボード DB |
| 失敗時の扱い | Notion 連携失敗は警告ログのみ。DB 保存は成功扱い |

### 14.2 Notion DB スキーマ

| プロパティ名 | タイプ | 書き込み内容 |
|---|---|---|
| 日付 | Title | `2026-04-25` 形式（YYYY-MM-DD） |
| プロジェクト | Select | `rate-time` |
| インプレッション | Number | `kpis.impressions` |
| クリック | Number | `kpis.clicks` |
| CTR | Number (Percent) | `kpis.ctr`（0.0833 のような小数） |
| 異常値 | Number | `anomalies.length` |
| 詳細リンク | URL | `https://rate-time.com/admin/reports/YYYY-MM-DD` |
| 保存日時 | Created time | Notion 自動設定 |

### 14.3 upsert 仕様

`src/lib/notion-client.ts` の `upsertDailyReportToNotion()` が担当する。

1. `databases.retrieve({ database_id })` で `data_sources[0].id` を解決（結果はプロセス内でキャッシュ）
2. `dataSources.query({ data_source_id, filter })` で `プロジェクト + 日付` による既存行検索
3. 既存行あり → `pages.update` で上書き
4. 既存行なし → `pages.create` で新規作成（parent に `data_source_id` を使用）
5. レスポンスに `{ pageId, created: boolean }` を返す

**Notion SDK バージョン:** `@notionhq/client@^5.0.0`

Notion API 2025-09-03 以降、データベースのクエリは `databases.query` が廃止され `dataSources.query` に移行した。
v5 SDK では `databases.retrieve` のレスポンスに `data_sources` 配列が含まれるため、
`database_id` → `data_source_id` の変換を起動時に1回実施する。
...
- ローカル: `.env.local` に手動追加
- 本番: Vercel Dashboard の Environment Variables で設定

両変数が未設定の場合、Notion 連携はスキップされ `notion: { ok: false, error: "env_not_configured" }` がレスポンスに含まれる。
...
```json
{
  ...
  "date": "2026-04-24",
  "id": 1,
  "saved_at": "2026-04-25T01:00:00.000Z",
  "notion": {
    "ok": true,
    "created": true
  }
...
`notion.created` は新規作成時 `true`、更新時 `false`。
